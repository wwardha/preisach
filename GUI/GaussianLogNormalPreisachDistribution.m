function [result, iter] = GaussianLogNormalPreisachDistribution(a0, sigmam, sigmac, x0, y0, nSpline, nc, iter, f)   
    ...
    HC = ((((X-x0)-(Y-y0))/2))/muc;
    DC = muc*(sqrt(2*pi)*sigmac)^(-1).*((((X-x0)-(Y-y0))/2)).^(-1);

    PHC = DC.*exp(-(log(abs(HC)).^2)/(2*(sigmac/muc)^2));
    PHC(~isfinite(PHC)) = 0;
