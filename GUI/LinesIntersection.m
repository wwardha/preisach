function [cmt] = LinesIntersection(a0, x0, y0, cm, group, ncircle, showBar, f)
    i=0;
    cmt=[];
    for g=1:group
        if (showBar)
            s1 = 'Calculate Hysterons Position...';
            s2 = num2str(floor((g/group)*100));
            s3 = '%';
            waitbar(g/group,f,[s1 s2 s3]);
        end
        cmf=find(cm(:,1)==g);
